...
def get_db():
    ...
    try:
        ...
        logging.critical("Database connection failed: " + e.__str__)
    finally:
        db.close()
...
def create_user(user: UserCreate, db: Session = Depends(get_db)):
    logging.info("Creating user " + user.email)
    crud = UserRepository(db)
    db_user = crud.get_user_by_email(email=user.email)
    if db_user:
        logging.warn("User " + user.email + " already exsists")
...
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    logging.debug("Getting " + users.count + " users")
    return users
...
def read_user(user_id: int, db: Session = Depends(get_db)):
    ...
    if db_user is None:
        logging.warning("User with id = " + user_id + " not found")
